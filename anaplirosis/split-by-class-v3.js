    ...
    group.members.forEach(member => {
      ...
      if (!classLetter) {
        Object.keys(member).forEach(key => {
          // Έλεγχος αν το key είναι τμήμα (π.χ. Α11, Β31, Γ22)
          if (/^[ΑΒΓABC][0-9]{2}$/i.test(key)) {
      ...
      if (!classLetter) {
        Object.values(member).forEach(value => {
          if (typeof value === 'string' && /^[ΑΒΓABC][0-9]{2}$/i.test(value)) {
...
console.log(`   • Τμήματα (${stats.A.departments.size}): ${Array.from(stats.A.departments).sort().join(', ')}`);
console.log('');
...
console.log(`   • Τμήματα (${stats.B.departments.size}): ${Array.from(stats.B.departments).sort().join(', ')}`);
console.log('');
...
console.log(`   • Τμήματα (${stats.C.departments.size}): ${Array.from(stats.C.departments).sort().join(', ')}`);
console.log('');
