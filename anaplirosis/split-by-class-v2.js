...
data.groups.forEach((group, index) => {
  ...
  // Μέθοδος 2: Έλεγχος από τον τίτλο της ομάδας
  if (!classLetter && group.title) {
    const titleMatch = group.title.match(/[ΑΒΓ][0-9]{2}/i);
    if (titleMatch) {
  ...
  if (classLetter === 'Α' || classLetter === 'A') {
    ...
    // Εμφάνιση πληροφοριών για debug
    console.log(`⚠️  Ομάδα ${index + 1} (${group.groupName}): Title="${group.title}", FirstMember="${group.members?.[0]?.['Τμήμα'] || 'N/A'}"`);
  }
});
...
console.log(`   • Τμήματα: ${Array.from(stats.A.departments).sort().join(', ') || 'N/A'}`);
console.log('');
...
console.log(`   • Τμήματα: ${Array.from(stats.B.departments).sort().join(', ') || 'N/A'}`);
console.log('');
...
console.log(`   • Τμήματα: ${Array.from(stats.C.departments).sort().join(', ') || 'N/A'}`);
console.log('');
